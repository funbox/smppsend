defmodule SMPPSend.ESMEHelpers do
  ...
  def connect(host, port, bind_pdu, session_opts \\ [], esme_mod \\ SMPPSend.ESMESync) do
    ...
    case esme_mod.start_link(host, port, session_opts) do
      ...
      {:error, reason} ->
        {:error, "error connecting: #{inspect(reason)}"}
    end
  end
    ...
    case response do
      {:ok, pdu} ->
        ...
        case Pdu.command_status(pdu) do
          ...
          status ->
            {:error, "bind failed, status: #{status}"}
        end

      :timeout ->
        {:error, "bind failed, timeout"}

      :stop ->
        {:error, "bind failed, esme stopped"}

      {:error, error} ->
        {:error, "bind failed, error: #{inspect(error)}"}
    end
  end
  ...
  def wait_dlrs(_esme, _message_ids, timeout, _esme_mod) when timeout <= 0,
    do: {:error, "timeout"}

  def wait_dlrs(esme, message_ids, timeout, esme_mod) do
    case wait_for_pdus(esme, esme_mod, timeout) do
      {_, :stop} ->
        {:error, "ESME stopped while waiting for dlrs"}
