defmodule SMPPSend.TlvParserTest do
  ...
  test "convert_tlvs: bad name" do
    assert  {:error, _, "tlv_messssage_payload_s"  } = convert_tlvs([{"tlv_messssage_payload_s", "foo"}])
  end
  ...
  test "convert_tlvs: hex value" do
    assert  {:ok, [tlvs: [{0x1234, "Hello world!"}]]} = convert_tlvs([{"tlv_x1234_h", "48656C6C6F20776F726C6421"}])
  end
  ...
  test "convert_tlvs: string value" do
    assert  {:ok, [tlvs: [{0x1234, "Hello world!"}]]} = convert_tlvs([{"tlv_x1234_s", "Hello world!"}])
  end
end
